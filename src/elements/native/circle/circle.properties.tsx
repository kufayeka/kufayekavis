...
import { Row, numberInput, selectInput, ColorInput } from "../../../designer/ui/components/properties/controls";
...
export function renderCircleProperties(ctxUnknown: unknown): React.ReactNode {
  ...
  const fill = typeof c.fill === 'string' ? { type: 'solid', color: c.fill } : c.fill;
  ...
  return (
    <div className="grid grid-cols-2 gap-2 items-center">
      ...
      <Row id={`${baseId}-fill-color`} label="Fill Color" control={<ColorInput id={`${baseId}-fill-color`} value={fill.color} onChange={(v) => engine.updateElement(c.id, { fill: v })} />} />
    </div>
  );
}
